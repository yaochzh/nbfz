...
<div class="easyui-layout" fit="true">
  ...
  title="${ftl_description}" actionUrl="${entityName?uncap_first}Controller.do?datagrid" idField="id"  queryMode="group">
   ...
   <#list columns as po>
    ...
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if>  formatter="yyyy-MM-dd" extendParams="editor:'datebox'" width="100"></t:dgCol>
    <#elseif po.showType == 'textarea'>
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if>  extendParams="editor:'textarea'" width="100"></t:dgCol>
    <#elseif po.showType == 'checkbox' >
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if>  extendParams="editor:'combobox'" width="100"></t:dgCol>
    <#elseif po.showType == 'radio'> 
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}"<#if  po.isQuery == 'Y'>query="true"</#if>  extendParams="editor:'combobox'" width="100"></t:dgCol>
    <#elseif po.showType == 'datetime'>
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if>  formatter="yyyy-MM-dd hh:mm:ss" extendParams="editor:'datebox'" width="100"></t:dgCol>
    <#elseif po.showType == 'list'> 
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if>  extendParams="editor:'combobox'" width="100"></t:dgCol>
    <#elseif po.type == 'int'> 
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if>  extendParams="editor:'numberbox'" width="100"></t:dgCol>
    <#elseif po.type == 'double'>
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if>  extendParams="editor:'numberbox'" width="100"></t:dgCol>
    <#elseif po.type == 'BigDecimal'>
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if>  extendParams="editor:'numberbox'" width="100"></t:dgCol>
    <#else> 
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if>  extendParams="editor:'text'" width="100"></t:dgCol>
    </#if> 
   </#list>
